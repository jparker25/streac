...
def kernel(values, spacing, bandwidth=25 / 1000):
    ...
    res = np.zeros(
        len(spacing)
    )  # Create empty grid that will store the SDF values, length of desired grid
    for j in range(
        len(values)
    ):  # Iterate through each data point and find the SDF via the Gaussian kernel
        res += gaussian(
            (spacing - values[j]), bandwidth
        )  # Evaluate the Gaussian kernel at every point for the given value
...
def avg_excitation_check(neuron, baselines, stimuli, percentile=99):
    ...
    )  # Array of all trial stimulus SDFs
    ...
    for baseline in baselines:  # Iterate through all baseline trials
        ...
        ):  # Iterate trough all bins in baseline
    ...
    neuron.trial_excitation_areas = np.asarray(
        sorted(neuron.trial_excitation_areas)
    )  # Sort all areas in ascending order
    # Determine the average threshold dependent on neuron.average_shuffling
    neuron.excitation_threshold = (
        np.percentile(neuron.trial_excitation_areas, percentile)
    ...
    for i in range(
        ...
        )  # Find where the bin edge starts and stops
        if neuron.excitation_threshold < np.trapz(
    ...
    if neuron.generate_plots:
        ax[0].hist(
            neuron.trial_excitation_areas if neuron.average_shuffling else bl_areas,
            bins=20,
            color="gray",
            edgecolor="k",
        ...
        ax[1].vlines(
            stimuli[0].bin_edges, 0, ax[1].get_ylim()[1], color="k", linestyle="dashed"
        )  # Draw vertical lines for bin edges on the SDF stimulus plot
        ax[1].set_xlabel("Time (s)")
        ax[1].set_ylabel("SDF (spikes/s)")  # Label the SDF axes
        plt.suptitle(
            "Average Baseline SDF Bin Area Histogram and Average Stimulus SDF"
        )  # Title the figure
...
def trial_excitation_check(
    neuron, baseline_train, stim_data_train, trial_direc, percentile=99
):
    """
