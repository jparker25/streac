...
def freq(spike_train, time):
    ...
    """
    return (
        len(spike_train) / time
    )  # firing rate is number of spikes divided by total time recorded


class spike_train:
    def __init__(self, spikes, time, bin_width, type, mu, sigma, generate_plots):
        """
        ...
        """
        self.generate_plots = generate_plots  # True if generate plots for analysis
        self.type = type  # Set attribute
        self.time = time  # Set attribute
        self.bin_width = bin_width  # Set attribute
        self.bin_edges = np.arange(
            0, self.time + self.bin_width, self.bin_width
        )  # Create bins by bin_width
        self.spikes = spikes  # Set spikes
        self.first_spike = (
            self.spikes[0] if len(self.spikes) > 0 else self.time
        )  # Find the first spike
        self.last_spike = (
            self.spikes[-1] if len(self.spikes) > 0 else self.time
        )  # Find the last spike
        ...
        self.avg_isi = (
            np.mean(np.diff(self.spikes)) if len(self.spikes) > 1 else 0
        )  # Calc average ISI and set attribute
        self.t = np.linspace(
            0, self.time, int(self.time * 1000)
        )  # Create time array as attribute
        self.sdf = (
            exch.kernel(self.spikes, self.t, bandwidth=sigma)
            if len(self.spikes) > 1
            else np.zeros(int(self.time * 1000))
        )  # Generate SDF
    ...
    def get_bins(self):
        ...
        for i in range(len(self.bin_edges) - 1):  # Iterate through all bins
            ...
            curr_bin = bin(
                ...
                self.spikes[
                    np.where(
                        (self.spikes >= self.bin_edges[i])
                        & (self.spikes < self.bin_edges[i + 1])
                    )
                ],
                i + 1,
                f"{self.save_direc}/bin_{i+1}",
            )
            curr_bin.set_name(
                f"{self.name}_bin_{i+1:02d}"
            )  # Set the name of the bin object
        ...
        if self.generate_plots:
            ...
            )  # Create a figure of the bin firing rate and CV
            ax[0].plot(
                self.bin_edges[:-1],
                [b.freq for b in self.bins],
                marker="o",
                color="blue",
            )  # Plot the bin firing rate
            ax[1].plot(
                self.bin_edges[:-1], [b.cv for b in self.bins], marker="o", color="blue"
            )  # Plot the bin CV
            ax[1].set_ylabel("Bin CV")
            ax[0].set_ylabel("Bin Firing Rate (Hz)")
            # Set the ylabels
            ax[1].set_xlabel("Time (s)")  # Set the xlabel
            plt.suptitle(
                f"{self.name} Firing Rate (top) and CV (bottom)"
            )  # Set the title
    ...
    def save_spikes(self):
        ...
        np.savetxt(
            f"{self.save_direc}/{self.name}.txt",
            self.spikes,
            delimiter="\t",
            newline="\n",
            fmt="%f",
            header=f"{self.name} spike times.",
        )
    ...
    def save_meta_data(self):
        ...
        """
        with open(
            f"{self.save_direc}/{self.name}_meta_data.txt", "w"
            ...
            for key in self.__dict__:  # Save all meta data except large arrays
                if (
                    ...
                    and key != "t"
                    and key != "bins"
                ):  # Only save metadata without these labels
    ...
    def save_data(self):
        ...
        np.savetxt(
            f"{self.save_direc}/sdf.txt",
            self.sdf,
            newline="\n",
            delimiter="\t",
            header="# SDF of Spike Train",
        )  # Save the SDF to text file
        np.savetxt(
            f"{self.save_direc}/isif.txt",
            self.isif,
            newline="\n",
            delimiter="\t",
            header="# ISIF of Spike Train",
        )  # Save the ISIF to text file
        if (
            len(self.spikes) > 1 and self.generate_plots
        ):  # If spikes are greater than 1, plot the SDF and ISIF
            self.plot_fcns()
        fileHandler = open(
            f"{self.save_direc}/{self.name}.obj", "wb"
        )  # Open the file for spike train object
    ...
    def plot_fcns(self):
        ...
        axe[0].scatter(
            self.spikes, np.zeros(len(self.spikes)), marker="|", color="k"
        )  # Plot spikes as vertical hashes
        axe[1].scatter(
            self.spikes, np.zeros(len(self.spikes)), marker="|", color="k"
        )  # Plot spikes as vertical hashes
        axe[0].set_xlabel("Time (s)")
        axe[0].set_ylabel("SDF [spikes/s]")  # Label the SDF axes
        axe[1].set_xlabel("Time (s)")
        axe[1].set_ylabel("ISI Function [s]")  # Label the SDF axes
        axe[0].vlines(
            self.bin_edges,
            axe[0].get_ylim()[0],
            axe[0].get_ylim()[1],
