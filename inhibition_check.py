...
def moving_mean(x, window=250):
    ...
    movmean = np.zeros(
        x.shape[0]
    )  # Create an array that is the same size as the input to store moving average
    for i in range(
        window // 2, x.shape[0] - window // 2
    ):  # Iterate through all points that have full half window around it
        movmean[i] = np.mean(
            x[i - (window // 2) : i + (window // 2) + (window % 2)]
        )  # Find the moving mean at the point in question
    for i in range(
        0, window // 2
    ):  # Iterate through points at the front and find moving mean
        movmean[i] = np.mean(
            x[0 : i + window // 2 + (window % 2)]
        )  # Find the moving mean at the point in question
    for i in range(
        x.shape[0] - window // 2, x.shape[0]
    ):  # Iterate through points at the back and find moving mean
        movmean[i] = np.mean(
            x[i - window // 2 :]
        )  # Fin the moving mean at the point in question
...
def isi_function(spikes, t, avg=250):
    ...
    interp = np.interp(
        t, spikes[:-1], np.diff(spikes)
    )  # Degree 1 interpolation onto 't' for spike times and ISIs
    if (
        isis[-1] <= tEnd
    ):  # If the last ISI is smaller than the time to the end of recording
        interp[t >= spikes[-1]] = (
            tEnd  # Set all times of interpolation after last spike to the total time remaining
        )
    else:  # If the last ISI is larger than the time to the end of the recording
        interp[t >= spikes[-1]] = np.mean(
            [tEnd, isis[-1]]
        )  # Average the last ISI and tEnd and set all values after last spike
    if isis[0] <= spikes[0]:  # If the first ISI is smaller than the first spike time
        interp[t <= spikes[0]] = np.mean(
            [spikes[0], isis[0]]
        )  # Set all times before the first spike to the average of the first spike and first ISI
    else:  # If the first ISI is larger than the first spike
        interp[t <= spikes[0]] = spikes[
            0
        ]  # Set all times before the first spike to the time of the first spike
    isif = moving_mean(
        interp, window=avg
    )  # Find the moving average of the interpolated ISI
...
def avg_inhibition_check(neuron, baselines, stimuli, percentile=99):
    ...
    if neuron.generate_plots:
        ...
        )  # Create figure that will have 2 rows and 1 column
    results = np.zeros(
        len(stimuli[0].bin_edges) - 1
    )  # Create empty array that will store the classifications of each bin

    isi_bls = np.asarray(
        [baseline.isif for baseline in baselines]
    )  # Grab all ISIFs for baseline
    isi_stims = np.asarray(
        [stimulus.isif for stimulus in stimuli]
    )  # Grab all ISIFs for stim
    avg_isi_stim = np.mean(isi_stims, axis=0)  # Determine average ISIF for stim

    sdf_stims = np.asarray(
        [stimulus.sdf for stimulus in stimuli]
    )  # Grab all SDFs for stim
    ...
    if neuron.isif_inhibition:  # If ISIF is being used go here
        for isi_bl in isi_bls:  # Iterate through all baseline trials
            for i in range(
                len(baselines[0].bin_edges) - 1
            ):  # Iterate through all bins in baseline
                # Find the start and stop spot of the baseline bin
                st, ed = (
                    np.where(baselines[0].t <= baselines[0].bin_edges[i])[0][-1],
                    np.where(baselines[0].t < baselines[0].bin_edges[i + 1])[0][-1],
                )
                bl_areas.append(
                    np.trapz(isi_bl[st : ed + 1], x=baselines[0].t[st : ed + 1])
                )  # Find the area of the baseline SDF of the bin and append to array

        neuron.trial_inhibition_areas = np.asarray(
            sorted(neuron.trial_inhibition_areas)
        )  # Sort all areas in ascending order
        # Determine the average threshold dependent on neuron.average_shuffling
        neuron.inhibition_threshold = (
            np.percentile(neuron.trial_inhibition_areas, percentile)
            if neuron.average_shuffling
            else np.percentile(bl_areas, percentile)
        )

        for i in range(
            len(stimuli[0].bin_edges) - 1
        ):  # Iterate through all bin edges of stimulus
            st, ed = (
                np.where(stimuli[0].t <= stimuli[0].bin_edges[i])[0][-1],
                np.where(stimuli[0].t < stimuli[0].bin_edges[i + 1])[0][-1],
            )  # Find the start and stops
            if neuron.inhibition_threshold < np.trapz(
                avg_isi_stim[st : ed + 1], x=stimuli[0].t[st : ed + 1]
                ...
                if neuron.generate_plots:
                    ax[1].fill_between(
                        stimuli[0].t[st : ed + 1],
                        avg_isi_stim[st : ed + 1],
                        color="blue",
                        alpha=0.5,
                    )  # Fill the area of the SDF that is beyond the percentile of the baseline areas
        if neuron.generate_plots:
            ax[0].hist(
                neuron.trial_inhibition_areas if neuron.average_shuffling else bl_areas,
                bins=20,
                color="gray",
                edgecolor="k",
            ...
            ax[0].set_xlabel("Baseline ISI Area Bins")
            ax[0].set_ylabel("Count")  # Label the histogram axes
            ax[1].plot(
                stimuli[0].t, avg_isi_stim, color="blue"
            )  # Plot the SDF as a function of t
            ax[1].vlines(
                stimuli[0].bin_edges,
                0,
                ax[1].get_ylim()[1],
                color="k",
                linestyle="dashed",
            )  # Draw vertical lines for bin edges on the SDF stimulus plot
            ax[1].set_xlabel("Time (s)")
            ax[1].set_ylabel("ISI Function")  # Label the SDF axes
        ...
        for baseline in baselines:  # Iterate through all baseline trials
            for bi in range(
                ...
                # Find the start and stop spot of the baseline bin
                st, ed = (
                    np.where(baseline.t <= baseline.bin_edges[bi])[0][-1],
                    np.where(baseline.t < baseline.bin_edges[bi + 1])[0][-1],
                )
                bl_areas.append(
                    np.trapz(baseline.sdf[st : ed + 1], x=baseline.t[st : ed + 1])
                )  # Find the area of the baseline SDF of the bin and append to array
        ...
        # Determine the average threshold dependent on neuron.average_shuffling
        neuron.inhibition_threshold = (
            np.percentile(neuron.trial_inhibition_areas, 100 - percentile)
            if neuron.average_shuffling
            else np.percentile(bl_areas, 100 - percentile)
        )
        ...
        for i in range(
            ...
            if neuron.inhibition_threshold >= np.trapz(
                avg_sdf_stim[st : ed + 1], x=stimuli[0].t[st : ed + 1]
            ):  # See if area of bin is smaller than the percentile of the baseline shuffled areas
                results[
                    i
                ] += 1  # Make the classification of the bin a 1 indicating it is inhibited
                if neuron.generate_plots:
                    ax[1].fill_between(
                        stimuli[0].t[st : ed + 1],
                        avg_sdf_stim[st : ed + 1],
                        color="blue",
                        alpha=0.5,
                    )  # Fill the area of the SDF that is smaller the percentile of the baseline areas
        if neuron.generate_plots:
            ax[0].hist(
                neuron.trial_inhibition_areas if neuron.average_shuffling else bl_areas,
                bins=20,
            ...
            ax[0].set_xlabel("Baseline SDF Area Bins")
            ax[0].set_ylabel("Count")  # Label the histogram axes
            ax[1].plot(
                stimuli[0].t, avg_sdf_stim, color="blue"
            )  # Plot the SDF as a function of t
            ax[1].vlines(
                stimuli[0].bin_edges,
                0,
            ...
            ax[1].set_xlabel("Time (s)")
            ax[1].set_ylabel("SDF (spikes/s)")  # Label the SDF axes
    if neuron.generate_plots:
        plt.suptitle(
            "Average Baseline ISI Bin Area Histogram and Average Stimulus ISI Function"
        )  # Title the figure
        makeNice([ax[0], ax[1]])
...
def trial_inhibition_check_sdf(
    neuron, baseline_train, stim_data_train, trial_direc, percentile=1
):
    """
    ...
    bl_sdf_fs = [
        exch.kernel(bl_spike, baseline_train.t) for bl_spike in bl_spikes
    ]  # Create SDF for all permtutaions
    bl_areas = (
        []
    ...
    for i in range(10):  # Loop through all the shuffles
        ...
        ):  # In each shuffle, loop through each bin
            st, ed = (
    ...
    for i in range(len(stim_data_train.bin_edges) - 1):  # Iterate through each bin
        ...
        if np.percentile(bl_areas, percentile) >= np.trapz(
    ...
    if neuron.generate_plots:
        ax[0].hist(
            bl_areas, bins=20, color="gray", edgecolor="k"
        )  # Create a histogram of all the baseline SDF areas
        ax[0].vlines(
            np.percentile(bl_areas, percentile),
            ax[0].get_ylim()[0],
            ax[0].get_ylim()[1],
            color="k",
            linestyle="dashed",
        )  # Draw a verticle line where the percentile threshold is on the histogram
        ax[0].set_xlabel("Shuffled Baseline SDF Area Bins")
        ax[0].set_ylabel("Count")  # Label the histogram axes
        ax[1].plot(
            stim_data_train.t,
            stim_data_train.sdf,
        ...
        ax[1].scatter(
            stim_data_train.spikes,
            np.zeros(len(stim_data_train.spikes)),
            marker="|",
            color="k",
        )
        ax[1].set_xlabel("Time (s)")
        ax[1].set_ylabel("SDF (spikes/s)")  # Label the SDF axes
        plt.suptitle(
            "Baseline SDF Bin Area Histogram and Stimulus SDF"
        )  # Title the figure
...
def trial_inhibition_check_isif(
    neuron, baseline_train, stim_data_train, trial_direc, shuffles=10, percentile=99
):
    """
    ...
    bl_isi = np.diff(baseline_train.spikes)  # Find the baseline ISI values
    bl_shuffles = [
        np.random.permutation(bl_isi) if x > 0 else bl_isi for x in range(shuffles)
    ]  # Shuffle the bl ISI values for shuffles number of times
    bl_spikes = [
        [
            (
    ...
    bl_isi_fs = [
        isi_function(bl_spike, baseline_train.t) for bl_spike in bl_spikes
    ]  # Create ISIF function for all permutations
    ...
    first_bin_edge = np.where(stim_data_train.bin_edges <= stim_data_train.spikes[0])[
        0
    ][
        -1
    ...
    for i in range(len(stim_data_train.bin_edges) - 1):  # Iterate through each bin
        if i == first_bin_edge:  # if at the first bin edge
            ...
            if stim_data_train.spikes[0] >= np.percentile(
                bl_areas, percentile
            ):  # if time to first bin is larger than threshold, inhibited
    ...
    if neuron.generate_plots:
        ...
        ax[0].set_xlabel("Shuffled Baseline ISI Area Bins")
        ax[0].set_ylabel("Count")  # Label the histogram axes
        ax[1].plot(
            stim_data_train.t,
        ...
        ax[1].set_ylabel("ISI Function")  # Label the SDF axes
        plt.suptitle(
            "Baseline ISI Bin Area Histogram and Stimulus ISI Function"
        )  # Title the figure
        makeNice([ax[0], ax[1]])
